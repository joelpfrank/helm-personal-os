...
function scanCurrentTree() {
  const paths = git(['ls-files', '--cached', '--others', '--exclude-standard', '-z'], { encoding: 'buffer' })
    .toString('utf8')
    .split('\0')
    .filter(Boolean);
  const findings = [];
  ...
  return { findings, scanned: paths.length, scope: 'candidate working tree' };
}
...
function main() {
  const result = process.argv.includes('--history') ? scanHistory() : scanCurrentTree();
