...
// macOS ships lsof in /usr/sbin, which non-login shells and launchd-spawned
// environments routinely omit from PATH. PATH stays first so a test or an
// operator override still wins; the absolute fallbacks only apply when the
// command is genuinely not found, so a present-but-unusable lsof still counts
// as uncertainty rather than silently resolving through a different binary.
const LSOF_COMMANDS = ['lsof', '/usr/sbin/lsof', '/usr/bin/lsof'];

function processCwd(pid) {
  let lsof;
  for (const command of LSOF_COMMANDS) {
    lsof = spawnSync(command, ['-a', '-p', String(pid), '-d', 'cwd', '-Fn'], {
      encoding: 'utf8', timeout: 5_000,
    });
    if (lsof.error?.code !== 'ENOENT') break;
  }
...
// Returns a human-readable reason the legacy service looks live, or null when
// the legacy root is safe to copy. The reason names the evidence that actually
// triggered the refusal, so an operator is not sent looking for a pidfile that
// was never the cause.
function legacyServiceRunning(fromRoot) {
  ...
  if (fs.existsSync(pidFile)) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      try {
        process.kill(pid, 0);
        return `pid ${pid} from ${pidFile} is alive`;
      } catch {}
    }
  }
  ...
  const ps = spawnSync('ps', ['-axo', 'pid=,command='], {
    encoding: 'utf8',
    timeout: 5_000,
    // Busy Hermes hosts can legitimately exceed Node's 1 MiB spawnSync
    // default. Buffer the complete table so pressure is not misclassified as
    // an uninspectable process list and therefore a running legacy service.
    maxBuffer: 16 * 1024 * 1024,
  });
  if (ps.status !== 0 || ps.error) return 'the process table could not be inspected with `ps`';
  for (const line of ps.stdout.split('\n')) {
    const match = line.match(/^\s*(\d+)\s+(.*)$/);
    if (!match || Number(match[1]) === process.pid) continue;
    const [, pid, command] = match;
    if (command.includes(entrypoint)) return `pid ${pid} is running ${entrypoint}`;
    if (!RELATIVE_ENTRYPOINT_PATTERN.test(command)) continue;
    ...
    if (cwd?.unavailable) {
      return `pid ${pid} runs ${RELATIVE_ENTRYPOINT} but lsof could not resolve its working directory`;
    }
    if (cwd == null) continue;
    if (physicalPath(path.resolve(cwd, RELATIVE_ENTRYPOINT)) === physicalPath(entrypoint)) {
      return `pid ${pid} is running ${RELATIVE_ENTRYPOINT} from ${fromRoot}`;
    }
  }
  return null;
}
...
function main() {
  ...
  const running = legacyServiceRunning(fromRoot);
  if (running) {
    fail(
      `refusing to migrate: the legacy Helm service appears to be running ` +
      `(${running}). Stop it first, then re-run.`,
    );
  }
