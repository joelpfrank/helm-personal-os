...
describe('CI workflow (.github/workflows/ci.yml)', () => {
  ...
  it('packages and uploads the portable archive plus checksum for private rehearsal', () => {
    ...
    assert.equal(upload.with?.name, 'Helm-portable-v0');
...
describe('README workflow status badges', () => {
  // A badge whose workflow file was renamed, or whose workflow never runs on
  // main, renders a permanent "no status" on the public landing page. Resolve
  // each badge against the real workflow instead of string-matching the README.
  const badges = [...readText('README.md').matchAll(
    /!\[[^\]]*\]\(https:\/\/github[.]com\/([^/]+\/[^/]+)\/actions\/workflows\/([^/)]+)\/badge[.]svg([^)]*)\)/g,
  )];

  it('publishes a badge for every workflow that runs on main', () => {
    const badged = new Set(badges.map(([, , file]) => file));
    const onMain = fs.readdirSync(path.join(ROOT, '.github/workflows'))
      .filter((file) => {
        const push = readYaml(`.github/workflows/${file}`).on?.push;
        return Array.isArray(push?.branches) && push.branches.includes('main');
      });
    assert.ok(onMain.length > 0, 'at least one workflow must run on main');
    assert.deepEqual([...badged].sort(), onMain.sort());
  });

  it('points every badge at the real repository, workflow file, and branch', () => {
    for (const [, repository, file, query] of badges) {
      assert.equal(repository, 'joelpfrank/helm-personal-os');
      assert.ok(
        fs.existsSync(path.join(ROOT, '.github/workflows', file)),
        `badge references missing workflow ${file}`,
      );
      assert.equal(query, '?branch=main', `badge for ${file} must pin the default branch`);
    }
  });
});
