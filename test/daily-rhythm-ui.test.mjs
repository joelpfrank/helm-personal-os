...
describe('Today copy - Daily Command Meeting', () => {
  ...
  it('CheckInHistory can render and filter midday check-ins', () => {
    const s = read('web/src/components/coach/CheckInHistory.jsx');
    // Without a label entry a saved midday check-in renders a blank heading.
    assert.match(s, /midday:\s*'[^']+'/, 'KIND_LABEL must cover midday');
    assert.match(s, /midday:\s*'#[0-9a-f]{6}'/i, 'KIND_COLOR must cover midday');
  ...
  it('groups the cadence cards semantically for screen readers', () => {
    ...
    assert.match(today, /<section className="today-cadence"[^>]*aria-labelledby=/,
      'the cadence group needs an accessible name');
