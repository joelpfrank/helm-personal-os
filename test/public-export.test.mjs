...
function publicationSafetyFindings(root, { allowInternalHermes = false } = {}) {
  const findings = [];
  function visit(directory) {
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (allowInternalHermes && (relative === '.hermes.md' || relative === '.hermes' || relative.startsWith('.hermes/'))) continue;
...
describe('export-public-source.sh', () => {
  ...
  it('scans every public text file in the working tree for private markers and machine paths', () => {
    assert.deepEqual(publicationSafetyFindings(ROOT, { allowInternalHermes: true }), []);
  });
});
