...
describe('release version identity', () => {
  it('uses the root version everywhere users and clients can observe it', () => {
    // The published identity is the plain label "v0"; this semver number is a
    // placeholder that exists only because npm manifests require one.
    ...
    assert.match(readText('CHANGELOG.md'), /^## \[v0\]/m);
    assert.match(readText('CHANGELOG.md'), /first public release/i);
  });
});
...
describe('lockfile and public governance metadata', () => {
  ...
  it('ships the complete verification scripts from the tracked public source', () => {
    const packaging = readText('scripts/package-helm.sh');
    assert.match(packaging, /scripts\/\*/);
    assert.match(packaging, /git ls-files/);
  });
});
