...
describe('migrate-state.mjs dry-run default', () => {
  ...
  it('inspects a large process table without treating buffer pressure as a live service', () => {
    const tmp = tmpdir();
    const legacy = makeLegacy(tmp);
    const dest = path.join(tmp, 'state');
    ...
    fs.writeFileSync(path.join(fakeBin, 'ps'), [
      ...
      'i=1',
      'while [ "$i" -le 12000 ]; do',
      "  printf '%s synthetic-worker-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\\n' \"$i\"",
      '  i=$((i + 1))',
      'done',
      '',
    ].join('\n'), { mode: 0o755 });
...
describe('migrate-state.mjs refusals', () => {
  ...
  it('still resolves cwd for an unrelated relative entrypoint when lsof is not on PATH', async (t) => {
    // macOS ships lsof in /usr/sbin, which is absent from many non-login and
    // launchd-spawned PATHs. Resolving it only through PATH turned every
    // unrelated `node server/src/index.js` process on the host into an
    // uninspectable cwd, so the fail-closed guard refused a migration whose
    // own legacy root was idle.
    const tmp = tmpdir();
    const legacy = makeLegacy(tmp);
    const unrelated = path.join(tmp, 'unrelated');
    const entrypoint = path.join(unrelated, 'server', 'src', 'index.js');
    fs.mkdirSync(path.dirname(entrypoint), { recursive: true });
    fs.writeFileSync(entrypoint, 'setInterval(() => {}, 1000);\n');
    const child = spawn(process.execPath, [path.join('server', 'src', 'index.js')],
      { cwd: unrelated, stdio: 'ignore' });
    t.after(() => {
      try { child.kill('SIGTERM'); } catch {}
      fs.rmSync(tmp, { recursive: true, force: true });
    });
    ...
    const dest = path.join(tmp, 'state');
    const res = run(['--from', legacy, '--to', dest], { PATH: '/usr/bin:/bin' });
    assert.equal(res.status, 0, res.stderr);
    assert.match(res.stdout, /dry[- ]run/i);
    assert.ok(!fs.existsSync(dest));
  });
