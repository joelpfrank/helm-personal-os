...
`scripts/export-public-source.sh` remains a fail-closed audit tool for comparing an older private source tree with the public allow-list. It is not a second development path: never export private source over this canonical checkout and never import private Git history. Reconcile a generally useful change by reimplementing and testing it here; keep a personal-only change in the external overlay.
