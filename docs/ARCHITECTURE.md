...
![Helm runtime architecture showing local browser, Express API, SQLite, provider gateway, remote AI profiles, and MCP client paths](assets/helm-architecture.png)

The [standalone architecture visual](helm-architecture.html) is generated from
the same deterministic launch-asset source as the PNG above.
...
In-app chat ──> provider gateway ──> selected remote provider or Claude Code
                         └────────> reduced in-process Helm MCP tools
```
...
The provider registry gives every profile a stable ID, authentication class,
model catalog, capability flags, readiness reason, normalized text/tool stream,
and safe error contract. Verified profiles cover Claude Code plus Anthropic,
OpenAI, Google Gemini, and OpenRouter APIs. Codex subscription and Gemini CLI
login are not represented as reusable API credentials.

API credentials enter through a write-only local route and are stored as
owner-only files outside SQLite when `HELM_STATE_DIR` is configured. Browser
responses expose configured/not-configured status only. Claude Code remains a
provider-owned CLI authentication path; Helm checks status without copying its
credential store. No-AI mode bypasses provider readiness while preserving core
records.

The gateway normalizes provider streaming and tool-call lifecycles. Provider
failures are mapped to a finite safe taxonomy; raw response bodies, stack traces,
and secret values are not returned to the browser or persisted in chat history.
...
Optional features may contact Anthropic, OpenAI, Google Gemini, OpenRouter and
its selected upstream model, Google Calendar, messaging providers, and
operator-configured MCP services. Each adds its own credentials, policies,
availability, and data-retention behavior. Core records remain usable without
enabling those integrations.
