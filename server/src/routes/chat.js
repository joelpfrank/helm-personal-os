...
// Any model in the picker is selectable. Derive the allow-list from MODELS so
// adding a model above automatically makes it available everywhere.
...
async function runApiTurnLoop({ id, send, systemPrompt, workingMessages, model, tools, simplifiedTools = false }) {
  ...
  for (let turn = 0; turn < 12; turn++) {
    const accumulator = makeAssistantAccumulator();
    const stream = streamMessages({ model, system: systemPrompt, messages: workingMessages, tools });
    for await (const evt of stream) {
      accumulator.onEvent(evt);
      if (evt.type === 'content_block_start') {
        if (evt.content_block?.type === 'tool_use') {
          send({ type: 'tool_start', index: evt.index, id: evt.content_block.id, name: evt.content_block.name });
        }
    ...
    if (toolUses.length === 0) {
      sql.insertMessage.run(id, 'assistant', JSON.stringify(assistantMsg.content));
      send({ type: 'done', stop_reason: assistantMsg.stop_reason, usage: assistantMsg.usage });
      return;
    }
    sql.insertMessage.run(id, 'assistant', JSON.stringify(assistantMsg.content));
    workingMessages.push({ role: 'assistant', content: assistantMsg.content });
    ...
    for (const tu of toolUses) {
      ...
      try {
        ...
        result = { type: 'tool_result', tool_use_id: tu.id, is_error: true, content: String(err?.message || err) };
        send({ type: 'tool_result', id: tu.id, ok: false, message: String(err?.message || err) });
      }
...
async function runSdkTurn({ id, send, systemPrompt, workingMessages, model, simplifiedTools = false }) {
  // Accumulator that flattens content blocks across all internal turns
  // into one assistant message for our DB. Tool calls Claude made
  // appear as `tool_use` blocks; their results were resolved by the SDK
  // and don't need to be persisted as separate user messages.
  const blocks = [];
  let current = null;          // block being built
  let pendingIsTool = false;   // is the in-progress block a tool_use?
  let stopReason = 'end_turn';
  let usage = null;

  const stream = streamMessages({ model, system: systemPrompt, messages: workingMessages, simplifiedTools });
  for await (const evt of stream) {
    if (evt.type === 'content_block_start') {
      ...
      if (b?.type === 'text') {
        ...
        current = { type: 'tool_use', id: b.id, name: b.name, input: {}, _partialJson: '' };
        pendingIsTool = true;
        send({ type: 'tool_start', index: blocks.length, id: b.id, name: b.name });
      } else {
        current = null;
      }
    } else if (evt.type === 'content_block_delta' && current) {
      if (evt.delta?.type === 'text_delta') {
        current.text = (current.text || '') + (evt.delta.text || '');
        send({ type: 'text_delta', text: evt.delta.text || '' });
      } else if (evt.delta?.type === 'input_json_delta') {
        current._partialJson += evt.delta.partial_json || '';
      }
    } else if (evt.type === 'content_block_stop' && current) {
      if (pendingIsTool) {
        try { current.input = current._partialJson ? JSON.parse(current._partialJson) : {}; }
        catch { current.input = {}; }
        delete current._partialJson;
        send({ type: 'tool_input', id: current.id, name: current.name, input: current.input });
        // The SDK will run the tool and feed the result back internally;
        // tag it as "ok" on the frontend so the chip resolves.
        send({ type: 'tool_result', id: current.id, ok: true });
      }
      blocks.push(current);
      current = null;
      pendingIsTool = false;
  ...
  sql.insertMessage.run(id, 'assistant', JSON.stringify(blocks));
  send({ type: 'done', stop_reason: stopReason, usage });
}
