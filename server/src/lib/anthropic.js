...
export function hasApiKey() {
  return Boolean(resolveProviderCredential('anthropic:api'));
}
...
export async function messagesCreate({ model, system, messages, tools, max_tokens }) {
  const value = resolveProviderCredential('anthropic:api');
  if (!value) throw new Error('ANTHROPIC_API_KEY not configured');
  const body = {
