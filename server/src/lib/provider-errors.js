...
const PUBLIC_MESSAGES = {
  auth: 'Provider authentication failed or expired. Check the configured provider profile, then try again.',
...
export function classifyProviderError(err) {
  ...
  let code = PROVIDER_ERROR_CODES.includes(err?.code) ? err.code : 'provider';
  if (code !== 'provider') {
    // A normalized internal failure already carries a closed taxonomy code.
  } else if (/ANTHROPIC_API_KEY not configured|not configured yet/i.test(raw)) {
    code = 'setup';
  } else if (
