...
import { completeProfileText } from './provider-completions.js';
import { completeClaudeCodeText } from './provider-claude-code-runtime.js';
import { ACTIVE_PROFILE_ID, assertAiEnabled, providerRegistry } from './providers/registry.js';
import { backendKindForProfile } from './providers/contract.js';

export const ACTIVE_PROFILE = providerRegistry.get(ACTIVE_PROFILE_ID);
// Compatibility alias for callers not yet migrated to profile ids.
export const BACKEND = backendKindForProfile(ACTIVE_PROFILE);
...
function getSdkMcpInstance({ simplified = false } = {}) {
  ...
  const s = new McpServer({ name: `helm-${key}`, version: '0.0.0' });
...
// Provider-neutral stream consumed by Coach. streamMessages remains available
// for legacy Anthropic-event consumers.
export async function *streamProviderMessages(request, profile = ACTIVE_PROFILE) {
  assertAiEnabled();
  yield* profile.stream(request);
}

// Title generation stays inside the selected provider/profile boundary.
export async function generateTitle({ prompt, maxTokens = 40 }) {
  assertAiEnabled();
  const text = BACKEND === 'sdk'
    ? await completeClaudeCodeText({ prompt })
    : await completeProfileText(ACTIVE_PROFILE, { prompt, maxTokens });
  return text?.trim().replace(/^["'`]+|["'`.!?]+$/g, '').slice(0, 60) || null;
}

// Tool-less utility completion for background reasoning. The SDK path invokes
// Claude Code without MCP servers; API profiles use their own normalized
// adapter and compatible catalog rather than falling through to Anthropic.
export async function completeText({ system, prompt, model = null, maxTokens = 400 }) {
  if (!prompt || !String(prompt).trim()) return '';
  try {
    assertAiEnabled();
    return BACKEND === 'sdk'
      ? await completeClaudeCodeText({ system, prompt, model: model || undefined })
      : await completeProfileText(ACTIVE_PROFILE, { system, prompt, model, maxTokens });
  } catch { return ''; }
}
