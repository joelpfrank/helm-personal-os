...
// Cache the finite, secret-free readiness object exposed by a normalized
// provider profile. Existing Claude status behavior remains above.
export function createProviderStatus({ profile, now = Date.now, cacheTtlMs = 30_000 } = {}) {
  if (!profile || typeof profile.getStatus !== 'function') throw new Error('provider profile is required');
  let cached = null;
  return {
