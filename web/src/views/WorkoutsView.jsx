...
export default function WorkoutsView() {
  ...
  return (
    <section className="workouts-view tracking-surface tracking-workouts" aria-labelledby="workouts-heading">
      <header className="tracking-page-header workouts-page-header">
        <div>
          <div className="tracking-kicker">{active ? 'Session in progress' : 'Training record'}</div>
          <h2 id="workouts-heading" className="tracking-page-heading">Workouts</h2>
          <p className="tracking-page-summary">Sets, rest, and history stay together while you train.</p>
        </div>
        {!active && <button type="button" className="tracking-primary-action primary" onClick={handleStartFresh}>Start workout</button>}
      </header>
      <div className="workouts-toolbar tracking-toolbar">
        <TrackingTabs
          ...
          tabs={TABS.map((id) => ({
            id,
            label: id === 'active' && active ? 'Active ●' : id[0].toUpperCase() + id.slice(1),
          }))}
        />
        <span style={{ flex: 1 }} />
      ...
      <div
        className="workouts-body"
        id={`workouts-panel-${tab}`}
        role="tabpanel"
        aria-labelledby={`workouts-tab-${tab}`}
      >
        {tab === 'active' && (
