...
export default function ChatView() {
  ...
  const readinessLabel = !status
    ? 'Checking Coach readiness'
    : status.configured
      ? `${status.backend || 'Coach'} ready`
      : 'Coach setup needed';
  ...
  return (
    <div className={`chat-view${collapsed ? ' sidebar-collapsed' : ''}`}>
      ...
      <div className="chat-main">
        ...
        <div className="coach-command-context" aria-live="polite">
          <span className={`coach-readiness${status?.configured ? ' is-ready' : ' is-waiting'}`}>
            <span aria-hidden="true" />{readinessLabel}
          </span>
          <span className="coach-boundary"><strong>Model interpretation.</strong> Tool activity below is the evidence of requested reads and writes.</span>
        </div>
        ...
        {chatUnavailable && (
          <div className="chat-banner err">
            ...
            <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('helm:open-ai-settings'))}>Open AI settings</button>
          </div>
        )}
        <ChatTranscript
          messages={messages}
