  ...
  return (
    <div className={`goal-card${goal.status === 'done' ? ' done' : ''}`}>
      <div className="goal-card-head">
        ...
        />
        <span className="goal-path">{HORIZON_LABEL[goal.horizon]}{children.length ? ` · ${children.length} next` : ''}</span>
  ...
  return (
    <div className="goals-tree">
      <header className="goals-intro">
        <div className="today-kicker">Direction into action</div>
        <h3>Goals</h3>
        <p>Open one outcome to inspect its proof, target date, obstacles, and the smaller commitments beneath it.</p>
      </header>
      <div className="goals-toolbar">
        <span className="muted small">{roots.length} active top-level {roots.length === 1 ? 'goal' : 'goals'}</span>
        <span style={{ flex: 1 }} />
        <label className="muted small" style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
          <input type="checkbox" checked={showDone} onChange={(e) => setShowDone(e.target.checked)} />
          show done
        </label>
        {!adding && <button type="button" className="primary" onClick={() => setAdding(true)}>New goal</button>}
      </div>
