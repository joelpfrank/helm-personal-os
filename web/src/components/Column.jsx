...
export default function Column({ column, onCardClick, onMoveCard }) {
    ...
    <div ref={setNodeRef} style={style} className="column" {...attributes}>
      ...
      <SortableContext items={cardIds} strategy={verticalListSortingStrategy}>
        <div className="column-cards">
          {(column.cards || []).map((c) => (
            <Card key={c.id} card={c} onClick={() => onCardClick?.(c)} onMoveCard={onMoveCard} />
          ))}
          {(!column.cards || column.cards.length === 0) && !composing && (
            <div className="column-empty"><strong>Clear lane</strong><span>Drop a task here or add the next step.</span></div>
          )}
        </div>
      </SortableContext>
      {composing ? (
