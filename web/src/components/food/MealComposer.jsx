...
export default function MealComposer({ defaultDate, onLog }) {
  ...
  return (
    <div className="meal-composer">
      ...
      <div className="meal-composer-row">
        <input type="number" inputMode="decimal" aria-label="Calories" placeholder="kcal" value={calories}
          onChange={(e) => setCalories(e.target.value)} style={{ width: 84 }} />
        <input type="number" inputMode="decimal" aria-label="Protein grams" placeholder="p (g)" value={protein}
          onChange={(e) => setProtein(e.target.value)} style={{ width: 72 }} />
        <input type="number" inputMode="decimal" aria-label="Carbohydrate grams" placeholder="c (g)" value={carbs}
          onChange={(e) => setCarbs(e.target.value)} style={{ width: 72 }} />
        <input type="number" inputMode="decimal" aria-label="Fat grams" placeholder="f (g)" value={fat}
      ...
      <div className="muted small">
        Nutrition values are optional estimates. Leave them blank if unknown · Enter saves
      </div>
    </div>
  );
}
