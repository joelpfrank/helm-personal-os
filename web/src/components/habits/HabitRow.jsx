...
export default function HabitRow({ habit, onIncrement, onDecrement, onOpen, onEdit, onSetOutcome }) {
  ...
  const stripe = habit.color || 'var(--action)';
  ...
  return (
    <div
      ...
      style={{ '--habit-mark': stripe }}
    >
      <button
        type="button"
        className="habit-row-main habit-row-open"
        onClick={onOpen}
        aria-label={`View details for ${habit.name}`}
      >
        <span className="habit-state-mark" aria-hidden />
        ...
        <div className="habit-row-text">
          ...
          <div className="habit-row-progress-text" aria-live="polite">
            {effective === 'failed'
