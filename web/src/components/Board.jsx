...
export default function Board({ board, onCardClick }) {
  ...
  function moveCardToAdjacentColumn(card, direction) {
    const sourceIndex = board.columns.findIndex((column) => column.id === card.column_id);
    const targetColumn = board.columns[sourceIndex + direction];
    if (!targetColumn) return;
    const last = targetColumn.cards[targetColumn.cards.length - 1];
    moveCard(card.id, targetColumn.id, last ? last.position + 1000 : 1000);
  }
        ...
        <div className="board">
          {board.columns.map((col) => (
            <Column
              ...
              onMoveCard={moveCardToAdjacentColumn}
            />
          ))}
          {addingColumn ? (
